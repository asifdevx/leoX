...
export default function LatestTransaction() {
  ...
  console.log("isSold", nfts.map((nft)=>nft.isSold))
          ...
          {nfts.map((nft, index) => (
            <div key={index} className="border rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300">
              ...
              <div className="p-4">
                ...
                <p className="text-[#afbec3] mt-2">{nft.owner}</p>
                <p className="text-[#afbec3] mt-2">{nft.seller}</p>
              {nft.isSold === false && 
                <p className="text-[#afbec3] mt-2">for sale</p>
